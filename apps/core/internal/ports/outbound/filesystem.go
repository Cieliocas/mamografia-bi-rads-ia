...
type FilesystemReader interface {
	...
	// returning the header metadata alongside the pixels so every frame can be
	// rendered with the correct VOI LUT / windowing.
	// Returns an error if frameIdx is out of bounds.
	ReadDICOMFrame(path string, frameIdx int) (*Pixels16, *DICOMMetadata, error)
}
